...
export default function RoutePlanner() {
  ...
  const handleSearch = (e) => {
    ...
    if (!GOOGLE_MAPS_EMBED_KEY) {
      console.error('VITE_GOOGLE_MAPS_EMBED_KEY is not set — add it to your .env file.');
      return;
    }
    ...
    setMapUrl(
      `https://www.google.com/maps/embed/v1/directions?key=${GOOGLE_MAPS_EMBED_KEY}&origin=${encodedOrigin}&destination=${encodedDest}&mode=${mode}`
    );
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto flex flex-col h-[calc(100vh-100px)]">
      <div>
        ...
        <p className="text-slate-500 mt-1">Live transit directions via Google Maps.</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        ...
        {!GOOGLE_MAPS_EMBED_KEY && (
          <p className="text-xs text-red-500 mt-3">
            Google Maps key missing — set VITE_GOOGLE_MAPS_EMBED_KEY in your .env file.
          </p>
        )}
      </div>
